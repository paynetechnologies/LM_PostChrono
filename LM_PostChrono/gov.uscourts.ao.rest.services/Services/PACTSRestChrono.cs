...
namespace gov.uscourts.ao.rest.pacts.Services
{
    public class PACTSRestChrono : IPACTSRestChrono 
    {
        ...
        public bool PostChrono(chronos chrono, string districtId)
        {
            ...
            try
            {
                if (!aoRestClient.DefaultConfiguration.additionalHeader.ContainsKey("authenticationToken"))
                {
                    aoRestClient.DefaultConfiguration.additionalHeader.Add("authenticationToken", sts.GetSTS());
                }

                if (!aoRestClient.DefaultConfiguration.additionalHeader.ContainsKey("districtId"))
                {
                    aoRestClient.DefaultConfiguration.additionalHeader.Add("districtId", districtId);
                }
                else 
                {
                    aoRestClient.DefaultConfiguration.additionalHeader["districtId"] = districtId;
                }

                return (aoRestClient.RestPostNonQuery<chronos>(resourceUri, chrono));
            }
            catch (Exception ex)
            {
                throw ex;
